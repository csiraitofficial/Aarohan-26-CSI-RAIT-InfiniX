...
import { fetchIncidents, fetchAlerts, SharedIncident, SharedAlert, clearAllIncidents, clearAllAlerts } from "@/lib/sharedDataSync";
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell } from 'recharts';
import { Car, AlertTriangle, Activity, Video, Zap, MapPin, Users, Clock, Building, Building2, Play, Square, RefreshCw, X } from "lucide-react";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const storeInterval = setInterval(async () => {
      try {
        // Fetch from backend API (cross-device sync)
        const backendIncidents = await fetchIncidents();
        const backendAlerts = await fetchAlerts();

        // Also load from localStorage (same-device)
        const localIncidents = TrafficDataStore.loadIncidents();
        const localAlerts = TrafficDataStore.loadAlerts();

        // Merge: backend first (newer SOS alerts), then local, dedupe by ID
        // Ensure backend incidents have required fields for IncidentCard
        const normalizedBackendIncidents = backendIncidents.map(inc => ({
          ...inc,
        ...
        const mergedIncidents = [
          ...normalizedBackendIncidents,
          ...localIncidents.filter(li => !normalizedBackendIncidents.find(bi => bi.id === li.id))
        ] as Incident[];

        const mergedAlerts = [
          ...backendAlerts,
          ...localAlerts.filter(la => !backendAlerts.find(ba => ba.id === la.id))
        ] as Alert[];

        setIncidents(prevIncidents => {
          const prevStr = JSON.stringify(prevIncidents.map(i => ({ id: i.id, status: i.status })));
          const newStr = JSON.stringify(mergedIncidents.map(i => ({ id: i.id, status: i.status })));
          if (newStr !== prevStr) return mergedIncidents;
          return prevIncidents;
        });

        setAlerts(prevAlerts => {
          if (JSON.stringify(mergedAlerts.map(a => a.id)) !== JSON.stringify(prevAlerts.map(a => a.id))) return mergedAlerts;
          return prevAlerts;
        });
      } catch (err) {
        console.warn("Failed to sync incidents/alerts:", err);
      }
    }, 3000); // Poll every 3 seconds for faster SOS updates
  ...
  const handleClearAlerts = () => {
    setAlerts([]);
    TrafficDataStore.clearAlerts();
    clearAllAlerts(); // Sync with backend
  };

  const handleClearIncidents = () => {
    setIncidents([]);
    TrafficDataStore.clearIncidents();
    clearAllIncidents(); // Sync with backend
  };
        ...
        <div className="flex gap-3">
          ...
          <div className="glass rounded-xl px-5 py-3 border border-primary/20">
            ...
            <span className="text-sm font-bold text-primary flex items-center gap-2">
              ...
              {connected ? t("dashboard.operational") : t("dashboard.disconnected")}
            </span>
          </div>
        </div>
      </div>
      ...
      <Card className="p-4 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 border-2 border-primary/20">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="text-sm font-medium text-muted-foreground">{t("dashboard.simulationSource")}:</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <p className="text-sm text-gray-500 dark:text-white/70 font-medium mb-2">
                {simRunning ? t("dashboard.totalQueue") : t("dashboard.totalVehicles")}
              </p>
        ...
        <div className="glass-card rounded-2xl p-6 bg-white dark:bg-gradient-to-br dark:from-orange-500/20 dark:to-orange-600/10 border-gray-100 dark:border-orange-500/20 hover:scale-105 transition-all duration-300 cursor-pointer group shadow-sm dark:shadow-none">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <p className="text-sm text-gray-500 dark:text-white/70 font-medium mb-2">
                {simRunning ? t("dashboard.spillbackSignals") : t("dashboard.activeIncidents")}
              </p>
              <h3 className="text-4xl font-bold text-gray-900 dark:text-white mb-1 animate-fade-in">
        ...
        <div className="glass-card rounded-2xl p-6 bg-white dark:bg-gradient-to-br dark:from-purple-500/20 dark:to-purple-600/10 border-gray-100 dark:border-purple-500/20 hover:scale-105 transition-all duration-300 cursor-pointer group shadow-sm dark:shadow-none" onClick={() => navigate(selectedTier === 'tier1' ? '/simulation-tier1' : '/simulation-tier2')}>
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <p className="text-sm text-gray-500 dark:text-white/70 font-medium mb-2">{t("dashboard.simulationStep")}</p>
              <h3 className="text-4xl font-bold text-gray-900 dark:text-white mb-1 animate-fade-in">
                {displayStep}
              </h3>
              <p className="text-xs text-gray-400 dark:text-white/60 mt-1">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {simData.events.accident_signals?.length > 0 && (
              <Badge variant="secondary" className="bg-orange-200 text-orange-800">
                🔥 {t("incidents.accidentAt", { signals: simData.events.accident_signals.join(', ') })}
              </Badge>
            )}
            {simData.events.rally_signals?.length > 0 && (
              <Badge variant="secondary" className="bg-purple-200 text-purple-800">
                🚶 {t("incidents.rallyAt", { signals: simData.events.rally_signals.join(', ') })}
              </Badge>
            )}
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <DashboardMap
            ...
            onClearFocus={() => setFocusedLocation(undefined)}
            incidents={incidents.filter(i => ['sos', 'pothole', 'accident'].includes(i.type) && i.status !== 'resolved')}
          />
        </div>
        ...
        <Card className="p-4 bg-gradient-card border-2 border-primary/20">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold">{t("incidents.activeTitle")}</h3>
            </div>
            <div className="flex items-center gap-2">
              <Button
                size="sm"
              ...
              {activeIncidents.length > 0 && (
                <Button
                  size="sm"
                  variant="ghost"
                  className="h-8 text-[10px] text-muted-foreground hover:text-red-500 gap-1 px-2"
            ...
            {activeIncidents.length === 0 ? (
              ...
              activeIncidents.slice(0, 3).map(incident => (
                <IncidentCard
                  key={incident.id}
                  incident={incident}
                  onAssignOfficer={() => navigate('/incidents')}
                  onUpdateStatus={() => navigate('/incidents')}
