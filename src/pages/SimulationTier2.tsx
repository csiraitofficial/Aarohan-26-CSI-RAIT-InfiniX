/**
 * Tier 2 District City Simulation Page
 * Medium traffic demand patterns with emergency scenario controls
 * 
 * Features:
...
import { Play, Square, AlertTriangle, Truck, Flag, Trash2, Building, Users, TrendingDown, MapPin, Map, Grid3X3 } from "lucide-react";
import { toast } from "sonner";

import { API_CONFIG } from "@/lib/apiConfig";

const API_BASE = API_CONFIG.TIER2; // Dynamic - works on laptop and mobile
...
export default function SimulationTier2() {
    ...
    const connect = useCallback(async () => {
        try {
            ...
            if (!res.ok) throw new Error('Failed to connect to Tier 2 District City server')
    ...
    const setEmergency = async () => {
        ...
        if (path.length < 2) {
            toast.error(t('simulations.tiers.scenarios.emergencyPathError', { defaultValue: 'Emergency path needs at least 2 signals (e.g., T2_S1, T2_S2)' }))
            return
        }
        try {
    ...
    const setAccident = async () => {
        ...
        if (blocked.length === 0) {
            toast.error('Enter at least one signal ID (e.g., T1_S5)')
            return
        }
        try {
    ...
    const setRally = async () => {
        ...
        if (blocked.length === 0) {
            toast.error('Enter at least one signal ID (e.g., T1_S3, T1_S4)')
            return
        }
        try {
        ...
        <div className="space-y-6 p-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold">{t('simulations.tiers.level2Title')}</h1>
                        <p className="text-muted-foreground">{t('simulations.tiers.level2Desc')}</p>
                    </div>
                </div>
