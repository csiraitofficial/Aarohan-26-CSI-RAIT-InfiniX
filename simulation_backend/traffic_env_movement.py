...
class TrafficEnvCore:
    ...
    def _init_state(self):
        # Entry signals: traffic spawns here (no upstream connections)
        # These are the natural entry points to the network
        entry_signals = {'S1', 'S13', 'S14', 'S15'}
        
        # queues: initialize queues ONLY at entry signals
        # Non-entry signals start EMPTY so traffic must flow naturally through network
        self._queues = {}
        for s in self.net.signals:
            sid = s.signal_id
            self._queues[sid] = {}
            for m in s.movements:
                if sid in entry_signals:
                    # Entry signals get initial vehicles to start flow
                    self._queues[sid][m["id"]] = float(np.random.poisson(2))
                else:
                    # Non-entry signals start empty - traffic will arrive via pipelines
                    self._queues[sid][m["id"]] = 0.0
    ...
    def step(self, actions: Dict[str, int]):
        ...
        # 3b) spawn return traffic at exit signals (two-way flow)
        self._spawn_exit_return_traffic()
    ...
    def _process_pipelines(self):
        ...
        for dst in self.signal_ids:
            ...
            # --- BEGIN PATCH: distribute arrivals using turning_ratios if available ---
            # Build movement id -> (from, to) mapping
            mov_by_from_to = {}
            for m in sig.movements:
                mov_by_from_to.setdefault(m.get("from", "NA"), []).append(m)

            # Get turning ratios if available
            turns = getattr(sig, "turning_ratios", {}) or {}

            if turns:
                # Split arrivals across approaches, then within approach by turning fractions
                n_apps = len(mov_by_from_to) if mov_by_from_to else 1
                per_app = arriving / float(n_apps)
                for app, movs in mov_by_from_to.items():
                    # get directional fractions for this approach
                    frac_map = turns.get(app, {})
                    if frac_map:
                        # allocate per direction then to movements matching that direction
                        for to_dir, frac in frac_map.items():
                            amt = per_app * float(frac)
                            # find movements from this app that go to to_dir
                            matched = [m for m in movs if (m.get("to") == to_dir)]
                            if matched:
                                per_mov = amt / float(len(matched))
                                for m in matched:
                                    mid = m.get("id")
                                ...
                                # fallback: evenly across all movs for this app
                                per_mov = amt / float(len(movs)) if movs else amt
                                for m in movs:
                                    mid = m.get("id")
                                    if mid not in self._queues[dst]:
                                        self._queues[dst][mid] = 0.0
                        ...
                        # no turning fractions for this approach — evenly distribute across movs
                        per_mov = per_app / float(len(movs)) if movs else per_app
                        for m in movs:
                            mid = m.get("id")
                            if mid not in self._queues[dst]:
                                self._queues[dst][mid] = 0.0
                ...
                # fallback to original equal distribution by approach+mov
                app_to_movs = {}
                for m in sig.movements:
                    frm = m.get("from", "NA")
    ...
    def _time_of_day_factor(self):
        """
        Realistic time-of-day demand variation.
        - Morning rush: steps 300-600 (simulating 8-9 AM)
        - Evening rush: steps 2400-2700 (simulating 5-6 PM)
        ...
        # Morning rush peak (1.8x demand)
        if 300 <= t <= 600:
            progress = (t - 300) / 300.0
            return 1.0 + 0.8 * math.sin(math.pi * progress)
        
        # Evening rush peak (1.6x demand)
        if 2400 <= t <= 2700:
            progress = (t - 2400) / 300.0
            return 1.0 + 0.6 * math.sin(math.pi * progress)
        
        # Base cyclic variation (1.0-1.3x)
        if self.demand_variation_mode == "cyclic":
            return 1.0 + 0.3 * math.sin(2 * math.pi * t / 1800.0)
        
        return 1.0

    def _spawn_exogenous_arrivals(self):
        """
        Spawn traffic at entry signals with staggered start.
        Entry signals: S1, S13, S14, S15
        S1 starts immediately, others join gradually to simulate
        realistic network fill.
        """
        tod = self._time_of_day_factor()
        
        # Staggered entry: different signals activate at different times
        # This creates a wave-like filling of the network
        entry_spawn_delay = {
            "S1": 0,     # Starts immediately (main corridor)
            "S13": 20,   # Starts at step 20
            "S14": 40,   # Starts at step 40
            "S15": 60,   # Starts at step 60
        }
        
        for sid in self.net.entry_signals:
            # Check if this entry signal should be active yet
            delay = entry_spawn_delay.get(sid, 0)
            if self._t < delay:
                continue  # Not yet active
            
            # Ramp-up factor for first 30 steps after activation
            time_since_active = self._t - delay
            ramp_factor = min(1.0, time_since_active / 30.0)
            ...
            for mtype, movs in type_to_movs.items():
                if not movs:
                    continue
                # Demand lambda per movement type: through > right > left
                if mtype == "through":
                    lam_base = 1.0
                elif mtype == "right":
                    lam_base = 0.4
                ...
                # Apply time-of-day factor and ramp-up
                lam = float(self.base_demand_level) * lam_base * tod * ramp_factor
    ...
    def _spawn_exit_return_traffic(self):
        """
        Exit signals generate return traffic flowing upstream.
        This creates realistic two-way flow in the network.
        Exit signals: S6, S12 (those with no downstream links)
        Traffic flows back into upstream signals via their pipelines.
        """
        if self._t < 30:  # Let network fill a bit first
            return
        
        tod = self._time_of_day_factor()
        return_rate = 0.3  # 30% of base demand as return traffic
        
        for exit_sid in self.net.exit_signals:
            # Get upstream links from this exit signal
            upstream_links = self.net.upstream.get(exit_sid, [])
            if not upstream_links:
                continue
            
            # Calculate return traffic amount
            lam = float(self.base_demand_level) * return_rate * tod
            
            # Distribute to upstream signals via their pipelines
            for link in upstream_links:
                upstream_sid = link.get("signal")
                tt = max(1, int(round(link.get("travel_time_s", 60))))
                
                if upstream_sid not in self._pipelines:
                    continue
                ...
                if n > 0:
                    # Schedule arrival to upstream signal
                    idx = min(tt - 1, len(self._pipelines[upstream_sid]) - 1)
                    if 0 <= idx < len(self._pipelines[upstream_sid]):
                        self._pipelines[upstream_sid][idx] += float(n)
    ...
    def _discharge_movements(self):
        ...
        for sid in self.signal_ids:
            ...
            for mid in allowed_movs:
                ...
                # --- BEGIN PATCH: smarter routing by movement 'to' direction + bearing match ---
                def _bearing(a_lat, a_lon, b_lat, b_lon):
                    # returns bearing in degrees from a -> b (0 = north, 90 = east)
                    import math
                    y = math.sin(math.radians(b_lon - a_lon)) * math.cos(math.radians(b_lat))
                    x = math.cos(math.radians(a_lat)) * math.sin(math.radians(b_lat)) - \
                        math.sin(math.radians(a_lat)) * math.cos(math.radians(b_lat)) * math.cos(math.radians(b_lon - a_lon))
                    brng = (math.degrees(math.atan2(y, x)) + 360) % 360
                    return brng

                def _bearing_to_cardinal(bearing_deg):
                    # Map bearing to cardinal: N (337.5-22.5), E (67.5-112.5), S (157.5-202.5), W (247.5-292.5)
                    if (bearing_deg >= 337.5 or bearing_deg < 22.5):
                        return "N"
                    if 22.5 <= bearing_deg < 67.5:
                        return "NE"
                    if 67.5 <= bearing_deg < 112.5:
                        return "E"
                    if 112.5 <= bearing_deg < 157.5:
                        return "SE"
                    if 157.5 <= bearing_deg < 202.5:
                        return "S"
                    if 202.5 <= bearing_deg < 247.5:
                        return "SW"
                    if 247.5 <= bearing_deg < 292.5:
                        return "W"
                    return "NW"

                # choose target link matching movement 'to' direction preferred
                mov_to_dir = mov_meta.get("to")  # e.g., "S", "E", etc.
                chosen_links = []
                if mov_to_dir:
                    # compute bearing from source to each downstream link's target and map to cardinal
                    src_lat, src_lon = getattr(sig, "lat", None), getattr(sig, "lon", None)
                    if src_lat is not None and src_lon is not None:
                        for link in dlinks:
                            tgt = link.get("signal")
                            tgt_sig = self._get_signal_by_id(tgt)
                            if tgt_sig is None:
                                continue
                            tgt_lat, tgt_lon = getattr(tgt_sig, "lat", None), getattr(tgt_sig, "lon", None)
                            if tgt_lat is None or tgt_lon is None:
                                continue
                            b = _bearing(src_lat, src_lon, tgt_lat, tgt_lon)
                            card = _bearing_to_cardinal(b)
                            # consider N/S/E/W as match
                            if card in (mov_to_dir,):
                                chosen_links.append(link)
                        # if none matched exact cardinal, pick the closest by angle to mov_to_dir
                        if not chosen_links:
                            # map requested mov_to_dir to central bearing
                            target_map = {"N": 0, "E": 90, "S": 180, "W": 270}
                            if mov_to_dir in target_map:
                                tgt_bearing = target_map[mov_to_dir]
                                best = None
                                best_diff = 360.0
                                for link in dlinks:
                                    tgt = link.get("signal")
                                    tgt_sig = self._get_signal_by_id(tgt)
                                    if tgt_sig is None or getattr(tgt_sig, "lat", None) is None:
                                        continue
                                    tgt_lat = getattr(tgt_sig, "lat")
                                    tgt_lon = getattr(tgt_sig, "lon")
                                    if tgt_lat is None or tgt_lon is None:
                                        continue
                                    b = _bearing(src_lat, src_lon, tgt_lat, tgt_lon)
                                    d = abs(((b - tgt_bearing + 180) % 360) - 180)
                                    if d < best_diff:
                                        best = link
                                        best_diff = d
                                if best is not None:
                ...
                # fallback to even distribution across all links if nothing chosen
                if not chosen_links:
                    chosen_links = dlinks

                # schedule arrival to chosen links only (divide routed_total among chosen links)
                n_chosen = len(chosen_links)
                per_chosen = routed_total / float(n_chosen) if n_chosen > 0 else 0.0
                for link in chosen_links:
                    ...
                    if 0 <= idx < len(self._pipelines[tgt]):
                        self._pipelines[tgt][idx] += per_chosen
                    else:
                        ...
                        if target_sig is None or not target_sig.movements:
                            continue
                        per_mov = per_chosen / float(len(target_sig.movements))
